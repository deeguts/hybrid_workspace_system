...
export const adminApi = {
  getAllUsers: () => request<UserRecord[]>("/api/admin/users"),

  getAllAttendance: () => request<AttendanceRecord[]>("/api/admin/attendance"),

  getDeficientEmployees: (monthYear: string) =>
    request<MonthlySummary[]>(`/api/admin/deficient?monthYear=${monthYear}`),

  approveLeave: (leaveId: number) =>
    request<LeaveRecord>(`/api/admin/leaves/${leaveId}/approve`, {
      method: "PUT",
    }),
  ...
  getAllManagers: () => request<UserRecord[]>("/api/admin/managers"),

  updateUserRole: (userId: number, role: "ADMIN" | "MANAGER" | "EMPLOYEE") =>
    request<UserRecord>(`/api/admin/users/${userId}/role?role=${role}`, {
      method: "PUT",
    }),

  assignManager: (userId: number, managerId: number | null) => {
    const query = managerId !== null ? `?managerId=${managerId}` : "";
    return request<UserRecord>(
      `/api/admin/users/${userId}/assign-manager${query}`,
      { method: "PUT" },
    );
  },
};
...
export const managerApi = {
  getTeamDeficiency: (monthYear: string) =>
    request<MonthlySummary[]>(
      `/api/manager/team-deficiency?monthYear=${monthYear}`,
    ),
};
