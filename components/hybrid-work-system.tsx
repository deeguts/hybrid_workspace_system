...
type View = 'calendar' | 'dashboard' | 'admin' | 'settings'
...
export default function HybridWorkManagementSystem() {
  ...
  // Redirect non-admins away from admin panel if they somehow land there
  useEffect(() => {
  ...
  // ── Loading splash (JWT rehydration) ────────────────────────────────────────
  if (isLoading) {
    return (
          ...
          {currentView === 'settings' && (
            <div className="p-8">
              <div className="bg-white rounded-xl border border-gray-200 p-8 max-w-lg">
                <h3 className="font-poppins text-xl font-bold text-blue-900 mb-2">Settings</h3>
